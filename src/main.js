...
window.addEventListener("DOMContentLoaded", () => {
  ...
  // ── Settings ─────────────────────────────────────────────────────────────
  // Settings is its own window. The list stays visible — a pairing QR is
  // useless if it covers the thing you were looking at — and the form applies
  // as it is edited, so this side only has to open the window and stay in
  // sync with what it writes.
  async function openSettings() {
    ...
    window.open("settings.html", "garia-settings", "width=780,height=600");
  }

  document.getElementById("open-settings-btn").addEventListener("click", openSettings);

  const listenSettings = window.__TAURI__?.event?.listen;
  if (typeof listenSettings === "function") {
    listenSettings("settings-changed", (event) => {
